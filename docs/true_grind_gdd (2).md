...
**Resolved during Milestone 33 review, but a much broader rule than armor alone — this affects every HP-restoration source in the game, not just equipment:** whenever HP regenerates, from *any* source, **Critical HP fills first, and only overflows into Main HP once Critical HP is completely full.** The reasoning: healing should prioritize getting a character out of danger (out of the Critical-warning state entirely) before topping off the buffer they'll spend down again in normal combat anyway. This is a real, retroactive audit item — **every existing HP-restoration source needs to be checked against this rule, not just future ones**: natural passive Health Regen (Milestone 5.5), Bed rest (Milestone 5), First Aid (Milestone 11), and Restoration Mage/Combat Medic's Heal, Regenerate, and Holy Nova (Milestone 24). None of these had an explicit fill-order specified before this — it's entirely possible some or all currently just fill Main HP only, or fill both simultaneously with no defined priority. This deserves its own dedicated audit pass, not a silent assumption that it already works this way everywhere.
...
### 4.1a Armor Proficiency — Light/Medium/Heavy (resolved design, deferred implementation, captured during Milestone 49 planning)

**A genuinely new mechanical category — nothing about wearing armor has ever leveled up before.** Every armor piece built so far (Milestones 28 and 33) is static equipment with flat stat bonuses. This captures a real future direction: Armor as a trainable proficiency, split into Light/Medium/Heavy weight classes, leveled through wear the way weapons/magic/gathering/crafting already work. Directly motivated by the Dragoon class (Lancer's evolution), which is meant to require a real Heavy Armor threshold.

- **Open design question, not yet resolved:** does wearing any armor train one generic proficiency, or does the specific weight class worn train its own distinct proficiency? The latter is more consistent with how magic schools already work (casting Fire Magic trains `fire_magic` specifically, not a generic "magic" stat) and is the more likely correct answer, but this needs a real decision when picked up, not an assumption.
- **Every existing armor piece would need retroactive tagging** with a weight class (Leather Cap, Leather Armor, Silk Cowl, Silk Robe, Bone Necklace, Wolf Claw Ring, Venom Charm) before this could function.
- **Confirmed explicitly as future work** — not to be built as a shortcut or stand-in just to unlock Dragoon; Dragoon should wait until this is designed and built properly.

**Armor equip/unequip rules, resolved during Milestone 28 review:** armor can only be equipped or unequipped **outside combat** — matching the same Outpost-only restriction already established for Active Class (Section 12) and Skill Loadout (Section 12.1), not a mid-dungeon action. **Unequipping subtracts the removed piece's HP bonus directly from current HP**, not just re-clamps current HP against the new (lower) max — a meaningfully harsher model than a simple clamp, chosen deliberately so armor actually matters when it's on. **If that subtraction would bring current HP to zero or below, current HP is floored at exactly 1 instead** — unequipping alone should never down or kill a character, even a badly-hurt one removing a large HP bonus.

**Corrected during Milestone 33 review: armor's HP bonus splits across both HP bars, not just Main HP.** A gap in the original Milestone 28 implementation only ever boosted Main HP, leaving Critical HP completely untouched by armor. **Default split is 50/50 across Main and Critical HP** — simple, predictable, and it directly fixes the actual gap by making both bars visibly benefit rather than a token amount going to one. **The split ratio is a per-item data property, not a hardcoded engine rule** — every current piece defaults to 50/50, but future specialized gear can weight differently (a "Vitality"-flavored item toward Main HP, a "Warding"-flavored item toward Critical HP) as a real itemization differentiator later, alongside the already-deferred physical/magical resistance roadmap. This retroactively applies to the four Milestone 28 pieces (Leather Cap, Leather Armor, Silk Cowl, Silk Robe), not just armor added from Milestone 33 onward.

**Corrected again, precision added: the floor-of-1 safety rule only needs to protect Critical HP, not Main HP.** Main HP dropping to 0 (or even functionally past 0) from an unequip is completely safe on its own — that's literally the definition of the Critical-warning state, not Downed. Downed only happens when *both* bars hit 0. So the actual safety requirement is narrower than "protect both bars": Critical HP must never be reduced to 0 by an unequip action (floored at exactly 1 instead), while Main HP is free to drop to 0 with no floor needed at all.
...
This is deliberately consistent with the hard-mode swarm-trap design decision — no safety net, real consequences for exploration choices, tension baked into the core loop rather than smoothed over. **Now scheduled as Milestone 40** — the debug-friendly instant-portal workflow has served its purpose across enough milestones that this is ready to build for real, and it's deliberately the last content milestone before a full integration audit.

**Retrofit decisions resolved during Milestone 40 planning, since several already-shipped systems implicitly assumed no real multi-floor descent would ever exist:**
- **Boss's every-5-floors counter (Milestone 34, corrected further in the tier-rarity fix) resets per-run, not as a persistent lifetime save counter.** It was explicitly built as a workaround given no real descent existed at the time; now that Continue provides real descent, reaching a guaranteed Boss floor means floor 5, 10, 15 of the *current run*, resetting to 0 each time a fresh descent begins from the Outpost.
- **Whether Elite/Epic/Boss rarity or Common enemy difficulty should scale with real floor depth is a genuine open decision for Milestone 40's plan to state explicitly**, not something assumed either way by this document.
- **Outpost-only restrictions (armor equip/unequip, Active Class swap, Skill Loadout swap) stay unavailable for the full duration of a Continue-chain** — confirmed as intentional, matching the same "no safety net mid-commitment" philosophy the Crystal itself embodies.

**Coupled addition, captured alongside the Crystal: an Emergency Teleport item (Alchemy).** Directly motivated by removing the free bidirectional portal — a craftable consumable that instantly returns the whole party to the Outpost, functioning identically to a Crystal's Return choice, usable only when the whole party is out of combat (never a mid-fight escape). Same testing-convenience-vs-final-design split already used for instant revive vs. the real item-based system: cheap during active development, but explicitly flagged as needing deliberately expensive ingredients once real alpha balancing begins — a cheap version of this item would quietly undermine the entire point of the one-way portal it exists around.
...
- **Skinning is strictly animal-type, resolved during Milestone 32 planning**: Wolf and Spider qualify, both confirmed — Spider Silk is genuinely Skinning-exclusive, not an open question. Humanoid/armored enemies (Orc Warrior, Void Knight) do not — their materials (Orc Heavy Hide, Void Plate) stay as automatic on-kill drops, untouched by this system.
- **Butchering harvests meat — from any enemy except Undead and Skeletons**, who obviously have no flesh to harvest. This exclusion needs to be a real per-enemy check, not an oversight waiting to happen the first time someone tries to butcher a Skeleton. Not narrowed to animals-only the way Skinning is — a Goblin or an Orc Warrior is Butchering-eligible even though neither is Skinning-eligible.
...
- **Real, deliberate retroactive consequence, resolved during Milestone 32 planning:** Wolf Pelt, Wolf Meat, and Monster Meat (Goblin) move exclusively to this corpse-harvest interaction, removed entirely from automatic on-kill drops. This means Armorsmithing's basic recipes (Milestone 28) and Cooking's original Herb Stew recipe (Milestone 10) lose their ingredient source without the corresponding new skill — so both stations' own Research Tree unlocks now require the relevant new skill's research completed first (Armorsmithing needs Skinning; Cooking needs Butchering), so a player can never unlock a station with nothing to craft. Blacksmithing is unaffected — none of its ingredients come from Skinning or Butchering-eligible materials.
- **Corpse nodes are inherently one-time, unlike the other four gathering types — but "one-time" means per eligible harvest type, not per corpse, corrected during Milestone 32 planning.** A corpse eligible for both Skinning and Butchering (Wolf being the clear example — it has both a pelt and meat) supports two independent harvest actions on the same corpse: skin it for the pelt, then separately butcher it for the meat. **Skinning never also yields meat, and Butchering never also yields a pelt** — each is its own distinct channel, own EXP, own resource, even though both draw from the same physical corpse. A corpse needs to track completion per eligible harvest type (e.g. `isSkinned`, `isButchered`), not one single depleted flag — it only becomes fully spent and non-interactive once every harvest type it's eligible for has actually been completed. **Order is enforced, confirmed after further review: Skinning must be completed before Butchering becomes available, on any corpse eligible for both** — you can't sensibly get at the meat before the hide is off. This gate only applies when a corpse is eligible for both; a corpse that's Butchering-eligible but was never Skinning-eligible in the first place (Goblin, for instance — no pelt at all) has no such prerequisite and is Butcherable immediately. This still fits the same "no respawn, gone once fully harvested" rule as every other node type; it just needed the completion condition and ordering stated precisely once dual-eligible enemies (like Wolf) were considered.
...
### 11.5a Elite/Epic/Boss Rarity — Corrected During Milestone 34 Review

**Real, retroactive correction to how Elite (Milestone 20) and Epic (Milestone 29) were actually implemented, not just a Boss-specific rule.** Both were added directly to the standard `enemyPool`, meaning they've spawned with roughly the same frequency as any other Common enemy in every room, every floor, since they shipped — the "zero-code integration" that made them easy to add also made them far too common. That needs correcting across all three tiers together, not just for Boss going forward:

- **Elite and Epic are removed from the standard `enemyPool`** used for normal per-room Common-enemy population. They no longer spawn as routine room fillers.
- **Both instead spawn via a separate, genuinely low-probability check** — a small percentage chance, evaluated per floor or per Heavy Combat room (exact mechanism and rate are implementation decisions, but the target feel is "a real surprise when it happens," not "expected most floors").
- **Boss is guaranteed roughly every 5th floor, plus a separate, low chance on other floors** — a persistent, cross-visit counter, since real multi-floor progression (the Teleporter Crystal system) doesn't exist yet. "Every 5 floors" currently means every 5th total floor generation across dungeon visits, not floors descended within one continuous run. This adapts cleanly once the real multi-floor system eventually lands — the same counter just starts counting descents within a run instead of separate visits.
- **Net effect:** Common enemies remain the default, expected encounter. Elite, Epic, and especially Boss should all feel like genuine rarities, not routine floor content.
- **Known, accepted tension, resolved during implementation:** with the proposed starting rates (Elite 12%, Epic 5%, per Heavy Combat room), Boss actually appears on a slightly higher percentage of floors (~21.6%) than Elite (~16.0%), purely because Boss's frequency comes from a guaranteed milestone trigger rather than pure chance. This is accepted as fine for now — the milestone-based trigger is explicitly a temporary approximation given single-floor-per-visit is a workaround for the still-deferred real multi-floor descent system. Once that system exists and "every 5 floors" means real descents within one continuous run rather than separate dungeon visits, a guaranteed Boss arrival every 5 floors will feel appropriately weighty on its own terms, regardless of how its raw percentage compares to Elite's. Not something to rebalance now.
